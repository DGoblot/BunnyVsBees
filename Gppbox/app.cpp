...
int main()
{
    ...
    while (window.isOpen())
    {
		...
		//Camera center handling

		//test for centering the camera in front of the player

		/*if (g.player->lookingLeft) CameraCenter = Vector2f(g.player->xx * C::GRID_SIZE - 1 * C::GRID_SIZE, g.player->yy * C::GRID_SIZE);
		else CameraCenter = Vector2f(g.player->xx * C::GRID_SIZE + 1 * C::GRID_SIZE, g.player->yy * C::GRID_SIZE);*/

		if (g.levelEditor.onoff) {
			v.setCenter(Vector2f(C::RES_X / 2, C::RES_Y / 2));
			if (!cameraChanged) {
				cameraChanged = true;
				v.zoom(C::PIXEL_SCALE);
			}
		}
		else {
			v.setCenter(g.cameraPosition);
			if (cameraChanged) {
				v.zoom(1 / C::PIXEL_SCALE);
				cameraChanged = false;
			}
		...
		//keep view up to date in case we want to do something with like... you know what.
